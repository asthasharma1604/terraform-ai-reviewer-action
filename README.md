...
Both `gemini_api_key` and `github_token` are required inputs. The action maps
these inputs to `GEMINI_API_KEY` and `GITHUB_TOKEN`, and populates `PR_NUMBER`
...
uses one Gemini API response for all three review modes. Because this file is
